...
export const Dashboard = () => {
  ...
  return <div dir={direction} lang={locale} className="min-h-screen">
    ...
    <main id="top" className="container pb-16 pt-10">
      <section className="hero"><div><span className="eyebrow"><Sparkles size={14} />{t.eyebrow}</span><h1>{t.search}</h1><p>{t.subtitle}</p></div><form onSubmit={analyze} className="search-box"><div className="search-field relative"><Github className="absolute start-4 top-3.5 text-[var(--muted)]" size={20} /><Input value={username} onChange={(event) => setUsername(event.target.value)} className="ps-12" placeholder={t.placeholder} aria-label={t.placeholder} dir="ltr" /></div><Button type="submit" disabled={loading}>{loading ? <LoaderCircle className="animate-spin" size={18} /> : <Search size={18} />}{t.analyze}</Button><Button type="button" variant="secondary" onClick={() => void loadDemo()}>{t.demo}</Button>{error && <p className="search-error text-sm text-red-500">{error}</p>}</form></section>
