...
const RepositoryCard = ({ repo, locale }: { repo: Repository; locale: Locale }) => {
  ...
  return <Card className="repo-card overflow-hidden"><CardContent>
    ...
    <div className="mb-4 flex flex-wrap gap-3 text-xs text-[var(--muted)]"><span className="flex items-center gap-1"><i className="size-2 rounded-full bg-violet-500" />{repo.language}</span><span className="flex items-center gap-1"><Star size={13} />{compactNumber(repo.stars, locale)}</span><span className="flex items-center gap-1"><GitFork size={13} />{compactNumber(repo.forks, locale)}</span><span className="flex items-center gap-1"><GitCommitHorizontal size={13} />{repo.commits}</span></div>
...
const ComparePanel = ({ repositories, locale }: { repositories: Repository[]; locale: Locale }) => {
  ...
  return <Card id="compare"><CardHeader><div><h2>{t.compare}</h2><p className="section-help">{locale === "fa" ? "پروژه‌ها را کنار هم و بر اساس معیارهای یکسان بسنجید" : "Evaluate projects side-by-side using the same signals"}</p></div><GitCompareArrows className="text-violet-500" /></CardHeader><CardContent>
    ...
    <div className="compare-grid"><strong>{first.name}</strong><span className="text-center text-xs text-[var(--muted)]">VS</span><strong className="text-end">{second.name}</strong>{metrics.map(([label, a, b]) => <div className="contents" key={label}><div><b>{compactNumber(a, locale)}</b><Progress value={a / Math.max(a, b) * 100} /></div><span className="text-center text-xs text-[var(--muted)]">{label}</span><div className="text-end"><b>{compactNumber(b, locale)}</b><Progress value={b / Math.max(a, b) * 100} color="#43c6db" /></div></div>)}</div>
  </CardContent></Card>;
};
...
export const Dashboard = () => {
  ...
  return <div dir={direction} lang={locale} className="min-h-screen">
    <header className="topbar"><div className="container flex h-16 items-center justify-between gap-4"><a href="#top" className="flex items-center gap-2 text-lg font-black"><span className="logo-mark"><Activity size={20} /></span>DevBoard</a><nav className={menu ? "is-open" : ""}>{t.nav.map((item, index) => <a key={item} href={["#overview", "#repositories", "#compare", "#activity"][index]} onClick={() => setMenu(false)}>{item}</a>)}</nav><div className="flex items-center gap-1"><Button variant="ghost" size="icon" onClick={() => setLocale(locale === "fa" ? "en" : "fa")} aria-label="Language"><Languages size={18} /></Button><Button variant="ghost" size="icon" onClick={() => setTheme(resolvedTheme === "dark" ? "light" : "dark")} aria-label="Theme">{resolvedTheme === "dark" ? <Sun size={18} /> : <Moon size={18} />}</Button><Button variant="ghost" size="icon" className="menu-btn" onClick={() => setMenu(!menu)} aria-label="Menu"><Menu size={20} /></Button></div></div></header>
    <main id="top" className="container pb-16 pt-10">
      <section className="hero"><div><span className="eyebrow"><Sparkles size={14} />GitHub intelligence</span><h1>{t.search}</h1><p>{t.subtitle}</p></div><form onSubmit={analyze} className="search-box"><div className="relative flex-1"><Github className="absolute start-4 top-3.5 text-[var(--muted)]" size={20} /><Input value={username} onChange={(event) => setUsername(event.target.value)} className="ps-12" placeholder={t.placeholder} aria-label={t.placeholder} /></div><Button type="submit" disabled={loading}>{loading ? <LoaderCircle className="animate-spin" size={18} /> : <Search size={18} />}{locale === "fa" ? "تحلیل" : "Analyze"}</Button><Button type="button" variant="secondary" onClick={() => void loadDemo()}>{t.demo}</Button>{error && <p className="w-full text-sm text-red-500">{error}</p>}</form></section>
      {loading && !analysis ? <div className="loading-state"><LoaderCircle className="animate-spin text-violet-500" size={30} /></div> : analysis && <>
        <section id="overview" className="profile-strip"><div className="flex min-w-0 items-center gap-4"><Image src={analysis.profile.avatarUrl} alt={analysis.profile.name} width={48} height={48} unoptimized /><div className="min-w-0"><h2 className="truncate">{analysis.profile.name}</h2><p className="truncate text-sm text-[var(--muted)]">@{analysis.profile.login} · {compactNumber(analysis.profile.followers, locale)} {t.followers}</p></div></div><span className="data-source"><i />{analysis.source === "demo" ? t.demoData : t.liveData}</span><div className="flex flex-wrap gap-2"><Button variant="secondary" size="sm" onClick={() => void exportPdf(analysis)}><Download size={15} />{t.exportPdf}</Button><Button variant="secondary" size="sm" onClick={() => void exportExcel(analysis)}><FileSpreadsheet size={15} />{t.exportExcel}</Button></div></section>
        <div className="mb-4 flex items-end justify-between"><div><h2 className="section-title">{t.overview}</h2><p className="section-help">{t.subtitle}</p></div><p className="hidden text-xs text-[var(--muted)] sm:block">{t.analyzed} {new Date(analysis.generatedAt).toLocaleString(locale)}</p></div>
        <section className="stats-grid"><StatCard icon={Code2} label={t.repositories} value={compactNumber(analysis.profile.publicRepos, locale)} accent="#7c6cff" /><StatCard icon={Star} label={t.stars} value={compactNumber(analysis.profile.totalStars, locale)} accent="#f3c84b" /><StatCard icon={GitCommitHorizontal} label={t.commits} value={compactNumber(analysis.profile.totalCommits, locale)} accent="#43c6db" /><StatCard icon={GitPullRequest} label={t.prs} value={compactNumber(analysis.profile.totalPullRequests, locale)} accent="#62d393" /></section>
        <section className="charts-grid" id="activity"><Card><CardHeader><div><h2>{t.activity}</h2><p className="section-help">{locale === "fa" ? "روند ۲۴ روز اخیر" : "Recent contribution momentum"}</p></div><Activity className="text-violet-500" /></CardHeader><CardContent><ActivityChart data={analysis.activity} /></CardContent></Card><Card><CardHeader><div><h2>{t.languages}</h2><p className="section-help">{locale === "fa" ? "بر اساس مخزن‌های اصلی" : "Across original repositories"}</p></div><Code2 className="text-cyan-500" /></CardHeader><CardContent><LanguageChart data={analysis.languages} /></CardContent></Card></section>
        ...
        <section id="repositories"><div className="mb-4"><h2 className="section-title">{t.repoHealth}</h2><p className="section-help">{locale === "fa" ? "امتیاز، وضعیت و پیشنهاد اختصاصی برای هر پروژه" : "Scores, checks and a focused recommendation for every project"}</p></div><div className="repos-grid">{analysis.repositories.map((repo) => <RepositoryCard key={repo.id} repo={repo} locale={locale} />)}</div></section>
        <section className="mt-7"><ComparePanel repositories={analysis.repositories} locale={locale} /></section>
      </>}
    </main>
    <footer><div className="container flex flex-col items-center justify-between gap-3 py-7 text-sm text-[var(--muted)] sm:flex-row"><span className="flex items-center gap-2"><span className="logo-mark small"><Activity size={14} /></span>DevBoard · Open source GitHub intelligence</span><a href="https://github.com/javad-hasani/DevBoard" className="hover:text-violet-500">GitHub ↗</a></div></footer>
  </div>;
};
